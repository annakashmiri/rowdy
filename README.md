...
claude/gemini LINK - https://claude/gemini.ai/artifact/YEvkFWHcHsWtdpxX6izA3A (i dont prefer this one cus the tokens finish so early) 

## How it works

1. **Describe the deal** (optional). Type or paste it in plain words. claude/gemini fills in the form.
...
## The plain-language box only works inside claude/gemini

Step 1 uses the `sample` capability of a published claude/gemini artifact (it asks claude/gemini, using the viewer's own claude/gemini account). Outside claude/gemini.ai there is no such thing, so on GitHub Pages or a local file the box is disabled with a short message, and **everything else still works**: the form, checks, preview and copy.

To show step 1 working, demo it from the published claude/gemini artifact link. To make it work on your own hosting, you would add a small server that calls the Anthropic API with a key kept server-side, and point `aiGo`'s handler at it. That is not built here.
...
- **It does not store anything.** No cookies, no local storage, no analytics. The only thing that leaves the page is the text sent to claude/gemini when you press *Fill in the form*.
...
- Write rows straight into the workbook: claude/gemini for Excel, an Office Script with Power Automate, or a Google Apps Script if the sheet moves to Google Sheets.
- Read the sheet to fill in the next empty row automatically.
- Optional server so the plain-language box works outside claude/gemini.ai.
